...
const MasterPage = () => {
  ...
  const loadCourses = async () => {
    try {
      setError("")
      setLoading(true)
      const data = await courseService.getAll("master")
      setCourses(data)
    } catch (err) {
      setError("마스터 강의를 불러오는 중 오류가 발생했습니다")
    } finally {
      setLoading(false)
    }
  }
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        >
          <div className="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-4">
            <div>
              <h1 className="text-4xl font-bold gradient-text mb-2">마스터 클래스</h1>
              <p className="text-gray-600 text-lg">
                최고 수준의 전문가 과정으로 마스터가 되어보세요
              </p>
            </div>
        ...
        >
          <div className="card-premium p-6 text-center">
            ...
            <p className="text-gray-600">총 마스터 클래스</p>
          </div>
          
          <div className="card-premium p-6 text-center">
            <div className="bg-primary/10 rounded-full w-12 h-12 flex items-center justify-center mx-auto mb-3">
              <ApperIcon name="Award" className="h-6 w-6 text-primary" />
            </div>
            <h3 className="text-2xl font-bold text-gray-900 mb-1">
              {courses.filter(c => c.isPinned).length}
            </h3>
            <p className="text-gray-600">고정된 클래스</p>
          </div>
          
          <div className="card-premium p-6 text-center">
            <div className="bg-accent/10 rounded-full w-12 h-12 flex items-center justify-center mx-auto mb-3">
              <ApperIcon name="BookOpen" className="h-6 w-6 text-accent" />
            </div>
            <h3 className="text-2xl font-bold text-gray-900 mb-1">
        ...
        >
          <VideoGrid
            ...
            emptyDescription="첫 번째 마스터 클래스를 업로드해보세요"
          />
        </motion.div>
